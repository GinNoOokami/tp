...
cPhs__Step dalv4CandleTag_c::create() {
    fopAcM_SetupActor(this, dalv4CandleTag_c);
    ...
    if (i_fopAcM_isSwitch(this, mSwOK)) {
        init_modeEnd();
    } else {
        init_modeWatch();
    }
...
void dalv4CandleTag_c::procMain() {
    typedef void (dalv4CandleTag_c::*action_func)();
    static action_func mode_proc[] = {
        &modeWatch,
        &modeNG,
        &modeNGWait,
        &modeEnd,
...
void dalv4CandleTag_c::modeWatch() {
    if (i_fopAcM_isSwitch(this, mSwOK)) {
        ...
        if (i_fopAcM_isSwitch(this, mSwNG)) {
            if (getEventID() != 0xff) {
...
void dalv4CandleTag_c::init_modeNG() {
    mTimer = l_HIO.mTimer;
    Z2GetAudioMgr()->seStart(Z2SE_OBJ_LV4_CNDL_MISS, NULL, 0, 0, 1.0f, 1.0f, -1.0f, -1.0f, 0);
    mMode = 1;
}
...
void dalv4CandleTag_c::modeNG() {
    if (mTimer != 0) {
        ...
        for (int i = 0; i < mSwTotal; ++i) {
            dComIfGs_offSwitch(sw_top, fopAcM_GetHomeRoomNo(this));
            sw_top++;
        }
        init_modeNGWait();
    }
...
void dalv4CandleTag_c::modeNGWait() {
    if (!dComIfGs_isSwitch(mSwNG, fopAcM_GetHomeRoomNo(this))) {
        init_modeWatch();
    }
}
...
static int dalv4CandleTag_Draw(dalv4CandleTag_c* i_this) {
    return static_cast<dalv4CandleTag_c*>(i_this)->Draw();
}
...
static int dalv4CandleTag_Execute(dalv4CandleTag_c* i_this) {
    return static_cast<dalv4CandleTag_c*>(i_this)->Execute();
}
...
static int dalv4CandleTag_Delete(dalv4CandleTag_c* i_this) {
    return static_cast<dalv4CandleTag_c*>(i_this)->Delete();
}
...
static int dalv4CandleTag_Create(fopAc_ac_c* i_this) {
    return static_cast<dalv4CandleTag_c*>(i_this)->create();
}
